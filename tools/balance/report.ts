...
import { loadPass1, measurePass2, pass2Section, reasoning2 } from './pass2.ts';
...
export interface ReportOptions {
  ...
  /** Seeds per arm for the second pass's A/B tests and focused node weights (re-runs use 3×). */
  readonly focusSeeds?: number;
  readonly careers?: number;
...
export async function buildReport(opts: ReportOptions = {}): Promise<string> {
  ...
  const pass1 = loadPass1();

  // The second pass's measurements; its snapshot is the report's "after".
  const pass2 = await measurePass2({
    label: 'second pass',
    seedCount,
    nodeSeeds: opts.focusSeeds ?? 200,
    careers: opts.careers ?? 12,
    workers,
  });
  const after = pass2.snapshot;
  ...
  // Every other node on the report's usual seeds; the nodes this pass targeted
  // come from its focused, larger measurement so the report has one verdict each.
  const focus = new Map(pass2.nodes.map((n) => [n.id, n]));
  const rest =
    nodeSeeds > 0
      ? await nodeWeights({
          seedCount: nodeSeeds,
          ...
          only: META_UPGRADES.map((d) => d.id).filter((id) => !focus.has(id)),
        })
      : [];
  const byId = new Map(rest.map((n) => [n.id, n]));
  const nodes = META_UPGRADES.map((d) => focus.get(d.id) ?? byId.get(d.id)).filter((n): n is NodeReport => !!n);
  ...
  out.push(
    `Generated by \`tools/balance\` (\`node tools/balance/run-balance.mjs report\`) in ${wall} s. ` +
      ...
      `(${pass2.nodeSeeds} for the second pass's A/B tests and the nodes it targeted), ${opts.careers ?? 12} whole careers.`,
  );
  out.push('');
  out.push(
  ...
  out.push('');
  out.push(pass2Section(pass1, pass2));
  out.push('');
  out.push(reasoning2(pass1, pass2));
  out.push('');
  ...
  out.push('');
  out.push('## First pass: before and after');
  out.push('');
  out.push(
    before && pass1
      ? `Kept for the record. "Before" is the content.ts the first pass started from, measured by the first pass's bot ` +
          `(\`before.json\`, ${before.seedCount} seeds per state); "after" is what it committed, measured by this pass's bot (\`pass1.json\`).`
      : 'No first-pass snapshots found (tools/balance/before.json, pass1.json).',
  );
  out.push('');
  if (pass1) out.push(beforeAfter(before, pass1.snapshot));
  out.push('');
  out.push(REASONING);
  return out.join('\n');
}

/** What the first pass changed in content.ts and why. Written by hand; the numbers above are measured. */
export const REASONING = `## First pass: what changed, and why
...
## Design concerns the first pass raised

Kept as written; the second pass's status is in brackets.

- **Sycophancy scales with max patience.** [Still open.] A press restores a fraction of the *max* bar, so Helpful, the patience upgrades and the patience cards all multiply it. Holding the no-infinite-patience line needed a 32 s cool-down, not DESIGN.md's "about 8 s". A restore of a flat share of the prompt's *base* patience would allow a short cool-down again.
- **Auto Mode cannot be a good trade while permission prompts clear in 8 clicks.** An active clicker clears one in about 1.5 s, so removing them only reshuffles the bad-incident pool, and rm -rf is added on top. Raising \`clearWithClicks\` on the permission incidents (to 25–40) would make the trade real; that is structure, outside this pass. [Addressed: see the second pass. Clicks alone were not enough, because Auto Mode replaces prompts rather than removing them.]
- **Pickups are the biggest patience and token source nobody tunes.** "thanks!" restores 20% of the bar and a Golden Token pays 20% of the requirement about every 27 s, so Serendipity (pickups every ≈17 s) is one of the strongest nodes in the tree. \`PICKUP_TUNING\` and pickup payloads were outside this pass. [Resolved in the second pass.]
- **DESIGN.md is now out of date in three places:** 👍 are two per prompt, not one; the sycophancy cool-down is 32 s; the tool ladder is no longer "the first game's curves" (content.ts says so in a comment too). [DESIGN.md has since been updated; the second pass made its ladder sentence stale again.]
- **The top of the ladder is thin.** Ralph Loop and Recursive Self-Improvement cost half a prompt's requirement for their first unit and pay back in about 60–150 s even on a maxed save, so they matter only on the last prompt or two. [Resolved in the second pass.]
- **Holding pays.** Leftover patience is worthless, so a competent player keeps buying until about 30% patience is left (the bonus-👍 line). A player who reports the moment the bar is green finishes faster and weaker. That is the wallet/report tension working, but the HUD could say more about it. [Still true.]
- **\`npm run balance\`** still points at game 1's \`tests/unit/balance.report.test.ts\`; it should run \`node tools/balance/run-balance.mjs report\` (or the new \`tests/unit/balance.targets.test.ts\`). [Resolved: it does now, and \`npm run balance:check\` runs the targets test.]
`;
